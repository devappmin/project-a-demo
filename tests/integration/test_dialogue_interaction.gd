...
func _test_document_event_resolution(adapter: Node, dialogue: DialogueService, view: DialogueView) -> void:
	...
	var reset_error: Variant = adapter.call("handle_action", &"inspect", {"dialogue_bundle_key":&"foundation.inspect", "dialogue_trigger_key":&"mirror.inspect"})
	assert_eq(reset_error, OK, "document payload resolves after a session reset")
	assert_eq(dialogue.current_node_id, &"default.start.line", "reset state selects the default event instead of the prior session event")
	dialogue.abort_dialogue(&"test_cleanup")
	GameSession.narrative_state.set_flag(&"mirror_seen", false)
