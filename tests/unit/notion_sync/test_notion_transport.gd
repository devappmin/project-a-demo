...
func _test_paginates_recorded_pages() -> void:
	_recorded_responses = [
		_response(200, {"object":"list", "results":[{"id":"page-1"}], "next_cursor":"cursor-2", "has_more":true, "type":"page_or_database", "page_or_database":{}}),
		_response(200, {"object":"list", "results":[{"id":"page-2"}], "next_cursor":null, "has_more":false, "type":"page_or_database", "page_or_database":{}})
	]
	_requests = []
...
func _test_unauthorized_response_hides_token() -> void:
	...
	_requests = []
	var transport := NotionTransport.new("test-token", Callable(self, "_request_recording"))
	var result: Dictionary = await transport.query_all("source-1", [])
