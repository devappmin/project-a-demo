...
func run() -> void:
	...
	assert_true(markdown.contains("## 중요 선택 자동 저장") and markdown.contains("자동 저장: 중요 선택"), "reference explains the approved important-choice authoring quote")
	...
	assert_eq(FileAccess.get_file_as_string("res://docs/narrative-state-reference.md"), markdown, "tracked narrative reference exactly matches rendered output")
