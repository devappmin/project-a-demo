...
이 선택을 마친 직후 반드시 자동 저장해야 한다면 같은 선택지 블록의 인용문에 다음 문구를 씁니다.

> 자동 저장: 중요 선택

이 문구가 없으면 자동 저장하지 않습니다. `자동 저장: 중요`, `중요함`처럼 등록되지 않은 다른 한국어 값은 추측하지 않고 가져오기 오류로 처리합니다. 자동 저장 표시는 선택지 항목 하나가 아니라 해당 선택지 블록 전체에 적용되며, 선택 결과와 이어지는 자동 진행이 안정된 대사 또는 다음 선택지에 도달한 뒤에만 저장 시점이 됩니다.
...
`기획안` 아래의 현재 작업 공간은 두 데이터베이스로 구성됩니다.

- `캐릭터 설정`: 캐릭터 한 명마다 한국어 산문 페이지 하나를 작성합니다. 캐릭터의 역할, 성격과 욕망, 말투, 관계, 표정, 이야기에서의 역할을 본문에 정리합니다.
- `장소 대화 문서`: 장소 하나 또는 서로 밀접하게 연결된 이야기 묶음마다 페이지 하나를 작성합니다. 실제 대사와 분기는 이 페이지 본문에서 작성합니다.

새 페이지는 각 데이터베이스의 `캐릭터 설정 템플릿` 또는 `장소 대화 템플릿`을 복제해 시작합니다. 템플릿의 한국어 제목과 본문을 채우면 되며, 내부 키는 요구되지 않습니다. `Dialogue Blocks`는 현재 작성 과정에 사용하지 않습니다. 관계 속성, 순서 번호, 영어 ID, JSON, 토큰 또는 live sync도 작성자가 관리하지 않습니다.

Notion 댓글은 질문·대안·검토를 위한 논의 공간입니다. 댓글에서 조건, 결과, 분기 규칙을 합의하면 해당 내용을 본문이나 인용 블록으로 옮겨 정식 규칙으로 남깁니다.

편집이 끝나면 Codex 또는 Claude에게 이 가이드의 정규화 요청문을 사용해 문서형 대화 규칙으로 정리하도록 요청합니다. 그다음 기존의 수동 dry-run 명령으로 결과를 먼저 검토하고, 합의된 경우에만 게시 명령을 실행합니다.
